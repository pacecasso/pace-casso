...
const outDir = path.join(root, "tmp-sneaker-component-labels", stamp);
...
function componentEdgeSet(comp) {
  return comp.edgeSet ?? new Set(comp.edgeIds);
}

function componentDegree(selectedByNode, comp, node) {
  const edgeSet = componentEdgeSet(comp);
  return (selectedByNode.get(node) ?? []).filter((e) => edgeSet.has(e.index)).length;
}

function componentEndpoints(selectedByNode, comp) {
  const endpoints = comp.nodes.filter((n) => componentDegree(selectedByNode, comp, n) === 1);
  return endpoints.length ? endpoints : comp.nodes;
}

function walkPathComponent(comp, selectedByNode, start) {
  const edgeSet = componentEdgeSet(comp);
  const degrees = comp.nodes.map((n) => componentDegree(selectedByNode, comp, n));
  const hasBranch = degrees.some((d) => d > 2);
  const endpointCount = degrees.filter((d) => d === 1).length;
  if (hasBranch || endpointCount > 2) return null;

  const used = new Set();
  const walk = [start];
  let current = start;
  while (used.size < comp.edgeIds.length) {
    const next = (selectedByNode.get(current) ?? [])
      .filter((e) => edgeSet.has(e.index) && !used.has(e.index))
      .sort((a, b) => b.hitRatio - a.hitRatio)[0];
    if (!next) break;
    used.add(next.index);
    current = next.a === current ? next.b : next.a;
    walk.push(current);
  }
  return used.size === comp.edgeIds.length ? walk : null;
}
...
function coverComponent(comp, selectedByNode, preferredStart = null) {
  const starts = componentEndpoints(selectedByNode, comp);
  const start = preferredStart && comp.nodes.includes(preferredStart) ? preferredStart : starts[0];
  return walkPathComponent(comp, selectedByNode, start) ?? dfsCoverComponent(comp, selectedByNode, start);
}

function dijkstraToAny(osm, start, goals) {
  if (goals.has(start)) return { ids: [start], meters: 0, goal: start };
  const dist = new Map([[start, 0]]);
  const prev = new Map();
  const open = new Map([[start, 0]]);
  const done = new Set();
  ...
  while (open.size && guard++ < 180000) {
    ...
    if (goals.has(cur)) {
      const ids = [cur];
      while (prev.has(ids[ids.length - 1])) ids.push(prev.get(ids[ids.length - 1]));
      return { ids: ids.reverse(), meters: best, goal: cur };
    }
    open.delete(cur);
    done.add(cur);
    for (const edge of osm.adj.get(cur) ?? []) {
...
function pushRouteId(routeIds, id) {
  if (routeIds[routeIds.length - 1] !== id) routeIds.push(id);
}

function buildSelectedByNode(selected) {
...
function componentCenter(osm, comp) {
  let sx = 0, sy = 0;
  for (const n of comp.nodes) {
    const [x, y] = llToLocal(osm.coord.get(n));
    sx += x;
    sy += y;
  }
  return [sx / comp.nodes.length, sy / comp.nodes.length];
}

function routeCoverFromStart(osm, selected, startIndex) {
  const selectedByNode = buildSelectedByNode(selected);
  const comps = selectedComponents(selected)
    .map((comp, i) => ({ ...comp, i, edgeSet: new Set(comp.edgeIds), center: componentCenter(osm, comp) }))
    .filter((c) => c.edgeIds.length >= 1);
  const routeIds = [];
  const connectors = [];
  const unresolved = [];
  const remaining = new Map(comps.map((c) => [c.i, c]));

  let currentComp = comps[startIndex] ?? comps[0];
  remaining.delete(currentComp.i);
  for (const id of coverComponent(currentComp, selectedByNode)) pushRouteId(routeIds, id);

  while (remaining.size) {
    const current = routeIds[routeIds.length - 1];
    const goals = new Map();
    for (const comp of remaining.values()) {
      for (const node of componentEndpoints(selectedByNode, comp)) goals.set(node, comp.i);
    }
    const connector = dijkstraToAny(osm, current, goals);
    if (!connector) {
      unresolved.push(...[...remaining.keys()]);
      break;
    }
    const nextComp = remaining.get(goals.get(connector.goal));
    remaining.delete(nextComp.i);
    for (const id of connector.ids.slice(1)) pushRouteId(routeIds, id);
    connectors.push({
      fromComp: currentComp.i,
      toComp: nextComp.i,
      meters: +connector.meters.toFixed(1),
      startNode: current,
      endNode: connector.goal,
    });
    for (const id of coverComponent(nextComp, selectedByNode, connector.goal).slice(1)) pushRouteId(routeIds, id);
...
function rankStartComponents(osm, comps) {
  const centers = comps.map((comp, i) => ({ i, center: componentCenter(osm, comp), edges: comp.edgeIds.length }));
  const xs = centers.map((c) => c.center[0]);
  const ys = centers.map((c) => c.center[1]);
  const minX = Math.min(...xs), maxX = Math.max(...xs);
  const minY = Math.min(...ys), maxY = Math.max(...ys);
  const nx = (x) => (x - minX) / Math.max(1, maxX - minX);
  const ny = (y) => (y - minY) / Math.max(1, maxY - minY);
  const picks = [
    [...centers].sort((a, b) => nx(a.center[0]) + ny(a.center[1]) - (nx(b.center[0]) + ny(b.center[1])))[0],
    [...centers].sort((a, b) => nx(a.center[0]) - nx(b.center[0]))[0],
    [...centers].sort((a, b) => nx(b.center[0]) - nx(a.center[0]))[0],
    [...centers].sort((a, b) => ny(a.center[1]) - ny(b.center[1]))[0],
    [...centers].sort((a, b) => b.edges - a.edges)[0],
  ].map((p) => p.i);
  return [...new Set(picks)];
}

function buildGreedyStreetCover(osm, selected) {
  const comps = selectedComponents(selected).filter((c) => c.edgeIds.length >= 1);
  const starts = rankStartComponents(osm, comps);
  const covers = starts.map((startIndex) => routeCoverFromStart(osm, selected, startIndex));
  covers.sort((a, b) => {
    const aLong = a.connectors.filter((c) => c.meters > 220).length;
    const bLong = b.connectors.filter((c) => c.meters > 220).length;
    const aMeters = a.connectors.reduce((sum, c) => sum + c.meters, 0);
    const bMeters = b.connectors.reduce((sum, c) => sum + c.meters, 0);
    return aLong - bLong || aMeters - bMeters || routeLengthKm(a.chain) - routeLengthKm(b.chain);
  });
  return { ...covers[0], candidates: covers.length };
}
...
async function renderBlindRoute(chain, file) {
  const local = chain.map(llToLocal);
  const b = bounds(local);
  const pad = 230;
  const w = 1100, h = 820;
  const view = { minX: b.minX - pad, minY: b.minY - pad, maxX: b.maxX + pad, maxY: b.maxY + pad };
  const scale = Math.min((w - 70) / (view.maxX - view.minX || 1), (h - 70) / (view.maxY - view.minY || 1));
  const usedW = (view.maxX - view.minX) * scale;
  const usedH = (view.maxY - view.minY) * scale;
  const ox = (w - usedW) / 2, oy = (h - usedH) / 2;
  const project = ([x, y]) => [ox + (x - view.minX) * scale, oy + (view.maxY - y) * scale];
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><rect width="100%" height="100%" fill="#fff"/><path d="${pathD(local, project)}" fill="none" stroke="#df7d25" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  await sharp(Buffer.from(svg)).png().toFile(file);
}
...
async function renderComponentLabels(osm, stats, file) {
  const pts = stats.flatMap((s) => s.edges.flatMap((e) => [llToLocal(e.ca), llToLocal(e.cb)]));
  const b = bounds(pts);
  const pad = 220;
  const view = { minX: b.minX - pad, maxX: b.maxX + pad, minY: b.minY - pad, maxY: b.maxY + pad };
  const w = 1400, h = 1000;
  const scale = Math.min((w - 80) / (view.maxX - view.minX), (h - 80) / (view.maxY - view.minY));
  const usedW = (view.maxX - view.minX) * scale;
  const usedH = (view.maxY - view.minY) * scale;
  const ox = (w - usedW) / 2, oy = (h - usedH) / 2;
  const project = ([x, y]) => [ox + (x - view.minX) * scale, oy + (view.maxY - y) * scale];
  const colors = ["#e31a1c", "#1f78b4", "#33a02c", "#ff7f00", "#6a3d9a", "#b15928", "#00a6c8", "#d81b60"];
  let paths = "";
  let labels = "";
  for (const s of stats) {
    const color = colors[s.i % colors.length];
    for (const e of s.edges) {
      paths += `<path d="${pathD([llToLocal(e.ca), llToLocal(e.cb)], project)}" fill="none" stroke="${color}" stroke-width="6" stroke-linecap="round" opacity="0.86"/>`;
    }
    const [lx, ly] = project([(s.bb.minX + s.bb.maxX) / 2, (s.bb.minY + s.bb.maxY) / 2]);
    labels += `<text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" font-family="Arial" font-size="18" font-weight="700" fill="#111" stroke="#fff" stroke-width="4" paint-order="stroke">${s.i}</text>`;
  }
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><rect width="100%" height="100%" fill="#fff"/>${paths}${labels}</svg>`;
  await sharp(Buffer.from(svg)).png().toFile(file);
}
function componentStats(comp, edgeByIndex, global) {
  const edges = comp.edgeIds.map((id) => edgeByIndex.get(id)).filter(Boolean);
  const pts = edges.flatMap((e) => [llToLocal(e.ca), llToLocal(e.cb)]);
  const bb = bounds(pts);
  const km = edges.reduce((sum, e) => sum + e.w, 0) / 1000;
  const cx = ((bb.minX + bb.maxX) / 2 - global.minX) / Math.max(1, global.maxX - global.minX);
  const cy = ((bb.minY + bb.maxY) / 2 - global.minY) / Math.max(1, global.maxY - global.minY);
  const width = bb.maxX - bb.minX;
  const height = bb.maxY - bb.minY;
  const horizontal = width / Math.max(1, height);
  return { comp, edges, km, bb, cx, cy, horizontal };
}
function mergeSelectedEdges(groups) {
  const byIndex = new Map();
  for (const group of groups) {
    for (const edge of group) {
      const existing = byIndex.get(edge.index);
      if (!existing || (edge.bestDistance ?? Infinity) < (existing.bestDistance ?? Infinity)) {
        byIndex.set(edge.index, edge);
      }
    }
  }
...
async function main() {
  ...
  const centerlinePixels = centerlineFromRouteMask(route.mask, route.bbox, image.info.width, image.info.height);
  const osm = buildOsm(JSON.parse(await fs.readFile(osmFile, "utf8")));
  const streetIndex = buildStreetIndex(osm.edges);
  const fit = { north: 40.7313, south: 40.7067, west: -74.0147, east: -73.9733, score: -2.908089 };
  const centerlineSelected = selectEdgesFromCenterline(osm, fit, centerlinePixels, streetIndex, 42);
  const selected = mergeSelectedEdges([centerlineSelected]);
  const edgeByIndex = new Map(selected.map((e) => [e.index, e]));
  const global = bounds(selected.flatMap((e) => [llToLocal(e.ca), llToLocal(e.cb)]));
  const stats = selectedComponents(selected).map((comp, i) => ({ i, ...componentStats(comp, edgeByIndex, global) }));
  const cover = buildGreedyStreetCover(osm, selected);
  const connectorMeters = cover.connectors.reduce((sum, c) => sum + c.meters, 0);
  const selectedMeters = selected.reduce((sum, e) => sum + e.w, 0);
  const longConnectors = cover.connectors.filter((c) => c.meters > 220);
  const veryLongConnectors = cover.connectors.filter((c) => c.meters > 420);
  const status = cover.unresolved.length || veryLongConnectors.length || connectorMeters > selectedMeters * 0.55 ? "needs_review" : "candidate";

  await renderScreenshotOverlay(sourceFile, selected, fit, route.mask, image.info.width, image.info.height, path.join(outDir, "1-selected-streets-on-screenshot.png"));
  await renderComponentLabels(osm, stats, path.join(outDir, "2-component-labels.png"));
  await renderStreetUnion(osm, selected, cover.chain, path.join(outDir, "3-street-union-with-route.png"), "selected streets plus exact GPX route");
  await renderBlindRoute(cover.chain, path.join(outDir, "4-blind-gpx-route.png"));
  await fs.writeFile(path.join(outDir, "recovered-route.gpx"), gpx("sneaker-street-cover", cover.chain));
  await fs.writeFile(path.join(outDir, "selected-streets.geojson"), JSON.stringify(geojsonEdges(selected), null, 2));
  await fs.writeFile(path.join(outDir, "connector-report.json"), JSON.stringify(cover.connectors, null, 2));
  await fs.writeFile(path.join(outDir, "component-stats.json"), JSON.stringify(stats.map((s) => ({ i: s.i, km: +s.km.toFixed(3), cx: +s.cx.toFixed(3), cy: +s.cy.toFixed(3), horizontal: +s.horizontal.toFixed(2), edges: s.edges.length, nodes: s.comp.nodes.length, bb: s.bb })), null, 2));
  await fs.writeFile(path.join(outDir, "summary.json"), JSON.stringify({
    status,
    selectedEdges: selected.length,
    selectedKm: +(selectedMeters / 1000).toFixed(2),
    components: stats.length,
    routeKm: +routeLengthKm(cover.chain).toFixed(2),
    routePoints: cover.chain.length,
    connectorCount: cover.connectors.length,
    connectorKm: +(connectorMeters / 1000).toFixed(2),
    connectorShareOfSelected: +(connectorMeters / Math.max(1, selectedMeters)).toFixed(3),
    longConnectorCount: longConnectors.length,
    veryLongConnectorCount: veryLongConnectors.length,
    longestConnectors: [...cover.connectors].sort((a, b) => b.meters - a.meters).slice(0, 20),
    unresolvedComponents: cover.unresolved,
    candidatesTried: cover.candidates,
    artifacts: {
      screenshotOverlay: "1-selected-streets-on-screenshot.png",
      componentLabels: "2-component-labels.png",
      streetUnionWithRoute: "3-street-union-with-route.png",
      blindRouteFromGpx: "4-blind-gpx-route.png",
      gpx: "recovered-route.gpx",
      selectedStreets: "selected-streets.geojson",
      connectors: "connector-report.json",
    },
  }, null, 2));
