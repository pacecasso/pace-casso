...
const outDir = path.join(root, "tmp-logo-proof", "strava-slow-checkpoint");
const M = 111320;
        ...
        for (const lower of [0.42, 0.54, 0.66, 0.78]) {
          for (const overlap of [0.08, 0.14, 0.22]) {
            ...
            const raw = [
              ...
              [sx * 0.79, 0], [sx * 0.5, sy * 0.55], [sx * 0.21, 0], [0, 0],
              [sx * (0.5 + overlap), -sy * 0.13], [sx * 0.5, -sy * lower], [sx * (0.5 - overlap), -sy * 0.13], [sx * (0.5 + overlap), -sy * 0.13],
            ];
