...
async function main() {
  ...
  const summary = [], sheet = [];
  let n = 0;
  for (const fit of FITS) for (const opts of optsList) {
    const chains = [];
    let failed = 0, detours = 0, anchors = 0;
    for (const pxStroke of strokes) {
      ...
      failed += routed.failed; detours += routed.detours; anchors += routed.anchors;
    }
    const id = `strokes-${String(++n).padStart(2, "0")}`;
    const dir = path.join(outDir, id);
    await fs.mkdir(dir, { recursive: true });
    const km = totalKm(chains);
    await render(osm, chains, path.join(dir, "map.png"), `${id} ${km.toFixed(1)}km f${failed} d${detours}`);
    await fs.writeFile(path.join(dir, "art-strokes.gpx"), gpxMulti(id, chains));
    const row = { id, fit: fit.id, opts, km: +km.toFixed(2), miles: +(km * 0.621371).toFixed(2), failed, detours, anchors, strokes: chains.length, map: path.relative(root, path.join(dir, "map.png")).replace(/\\/g, "/"), gpx: path.relative(root, path.join(dir, "art-strokes.gpx")).replace(/\\/g, "/") };
    summary.push(row);
