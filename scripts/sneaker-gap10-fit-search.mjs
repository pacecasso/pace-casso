...
const outDir = path.join(root, "tmp-sneaker-gap10-fit", stamp);
...
function gpx(name, chain) {
  return `<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="PaceCasso gap10 sneaker fit" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name><trkseg>\n${chain.map(([lat, lng]) => `<trkpt lat="${lat.toFixed(7)}" lon="${lng.toFixed(7)}"></trkpt>`).join("\n")}\n</trkseg></trk></gpx>\n`;
}
...
async function extractScreenshotRoute() {
  ...
  const closedComponents = prepareTracedBinaryComponents(lineMask, cw, ch, 10, 20);
