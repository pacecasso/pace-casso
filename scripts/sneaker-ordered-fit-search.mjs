...
const outDir = path.join(root, "tmp-sneaker-ordered-fit", stamp);
...
function gpx(name, chain) {
  return `<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="PaceCasso ordered sneaker fit" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name><trkseg>\n${chain.map(([lat, lng]) => `<trkpt lat="${lat.toFixed(7)}" lon="${lng.toFixed(7)}"></trkpt>`).join("\n")}\n</trkseg></trk></gpx>\n`;
}
...
async function extractScreenshotRoute() {
  ...
  const order = [5, 1, 3, 6, 0, 7, 2, 4].filter((i) => polylines[i]);
  function rotateClosed(path, target) {
    let best = 0;
    let bestD = Infinity;
    const open = path.slice(0, -1);
    for (let i = 0; i < open.length; i++) {
      const d = target ? dist(open[i], target) : open[i][0] * 1000 + open[i][1];
      if (d < bestD) {
        bestD = d;
        best = i;
      }
    }
    const out = open.slice(best).concat(open.slice(0, best));
    out.push(out[0]);
    return out;
  }
  function orient(path, current) {
    if (!path || path.length < 2) return [];
    const closed = dist(path[0], path[path.length - 1]) < 2.5;
    if (closed) return rotateClosed(path, current);
    if (!current) return path;
    const a = dist(path[0], current);
    const b = dist(path[path.length - 1], current);
    return b < a ? [...path].reverse() : path;
  }
  const joined = [];
  let current = null;
  for (const i of order) {
    const piece = orient(polylines[i], current);
    for (const p of piece) {
      const last = joined[joined.length - 1];
      if (!last || dist(last, p) > 1.2) joined.push(p);
    }
    current = joined[joined.length - 1] ?? current;
  }

  const points = simplifyByDistance(joined, 1.8);
  const b = bounds(points);
  const width = b.maxX - b.minX || 1;
  const unit = points.map(([x, y]) => [(x - b.minX) / width, (y - b.minY) / width]);
