...
const outDir = path.join(root, "tmp-logo-proof", "fresh-three-compile-only");
...
async function main() {
  ...
  for (const d of designs) {
    ...
    for (const scale of [0.9, 1.05, 1.22, 1.42]) {
      ...
      const result = compileContourToLattice(pts.map(toLatLng), graph, d.opts);
      if (!result) continue;
    ...
    const metrics = `${best.result.km.toFixed(1)} km, ${best.result.legCount} street legs, compile-only street lattice, skipped pins ${best.result.skippedPins}`;
