...
const outDir = path.join(root, "tmp-sneaker-silhouette-first", stamp);
...
async function extractSneakerDesign() {
  ...
  const routePoints = simplifyByDistance(silhouette, 3.5);
  return { crop, routePoints, featureStrokes: [silhouette], normalized: normalizeDesign(routePoints, [silhouette]) };
}
...
async function runCity(latticePath) {
  ...
  const rotations = [-45, -35, -25, -15, -8, 0, 8, 15, 25, 35, 45];
  const widths = [1200, 1600, 2100, 2700, 3500, 4500, 5800];
  const fabric = [];
  let scanCount = 0;
