...
const outDir = path.join(root, "tmp-sneaker-topology-prune", stamp);
...
function renderRouteOnly(chain, file, label = "") {
  ...
  const d = pathD(route, project);
  const text = label ? `<text x="32" y="46" font-family="Arial" font-size="22" font-weight="700" fill="#111">${label}</text>` : "";
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><rect width="100%" height="100%" fill="#fff"/><path d="${d}" fill="none" stroke="#111" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/>${text}</svg>`;
...
function componentStats(comp, edgeByIndex, global) {
  ...
  if (km >= 0.08) score += 4;
  if (km >= 0.14) score += 7;
  if (horizontal > 2.6) score += 5;
  if (cx > 0.7 && cy > 0.25 && cy < 0.7) score += 4;
  if (cy < 0.2 || cy > 0.86) score += 3;
  return { comp, edges, km, bb, cx, cy, horizontal, score };
}function stitchSelectedWalkMode(osm, selected, mode = "original") {
  if (mode === "original") return stitchSelectedWalk(osm, selected);
  const selectedByNode = new Map();
  for (const e of selected) {
    if (!selectedByNode.has(e.a)) selectedByNode.set(e.a, []);
    if (!selectedByNode.has(e.b)) selectedByNode.set(e.b, []);
  ...
  const comps = selectedComponents(selected);
  const centerOf = (comp) => {
    const pts = comp.nodes.map((n) => llToLocal(osm.coord.get(n)));
    const b = bounds(pts);
    return [(b.minX + b.maxX) / 2, (b.minY + b.maxY) / 2];
  };
  const compCenter = new Map(comps.map((c) => [c, centerOf(c)]));
  const nodeDegreeInComp = (comp, n) => (selectedByNode.get(n) ?? []).filter((e) => comp.edgeIds.includes(e.index)).length;
  const endpointsOf = (comp) => comp.nodes.filter((n) => nodeDegreeInComp(comp, n) === 1);
  const pickStartNode = (comp, prefer) => {
    const nodes = endpointsOf(comp).length ? endpointsOf(comp) : comp.nodes;
    let best = nodes[0], bestVal = Infinity;
    for (const n of nodes) {
      const [x, y] = llToLocal(osm.coord.get(n));
      let val = 0;
      if (prefer === "left") val = x;
      else if (prefer === "right") val = -x;
      else if (prefer === "bottom") val = y;
      else if (prefer === "top") val = -y;
  ...
  const routeIds = [];
  let current = null;
  let firstPrefer = mode === "rightToLeft" ? "right" : mode === "bottomFirst" ? "bottom" : "left";
  while (remaining.length) {
    let compIndex = 0;
    let start = pickStartNode(remaining[0], firstPrefer);
    let best = Infinity;
    if (current === null) {
      for (let i = 0; i < remaining.length; i++) {
        const [cx, cy] = compCenter.get(remaining[i]);
        let val = cx;
        if (mode === "rightToLeft") val = -cx;
        if (mode === "bottomFirst") val = cy * 2 + cx * 0.15;
        if (mode === "topFirst") val = -cy * 2 + cx * 0.15;
        if (val < best) { best = val; compIndex = i; }
      }
      start = pickStartNode(remaining[compIndex], firstPrefer);
    } else {
      const curLl = osm.coord.get(current);
      for (let i = 0; i < remaining.length; i++) {
        const nodes = endpointsOf(remaining[i]).length ? endpointsOf(remaining[i]) : remaining[i].nodes;
        for (const n of nodes) {
          const d = meters(curLl, osm.coord.get(n));
          const [cx, cy] = compCenter.get(remaining[i]);
          const [curX, curY] = llToLocal(curLl);
          let penalty = 0;
          if (mode === "leftToRight") penalty = Math.max(0, curX - cx) * 0.25;
          if (mode === "rightToLeft") penalty = Math.max(0, cx - curX) * 0.25;
          if (mode === "banded") penalty = Math.abs(cy - curY) * 0.18;
          if (d + penalty < best) { best = d + penalty; compIndex = i; start = n; }
        }
      }
    }
    const comp = remaining.splice(compIndex, 1)[0];
    if (current !== null && current !== start) {
      const connector = dijkstra(osm, current, start);
      if (connector) for (const id of connector.slice(1)) if (routeIds[routeIds.length - 1] !== id) routeIds.push(id);
    }
    const compWalk = dfsCoverComponent(comp, selectedByNode, start);
    for (const id of compWalk) if (routeIds[routeIds.length - 1] !== id) routeIds.push(id);
    current = routeIds[routeIds.length - 1] ?? current;
  }
  return routeIds.map((id) => osm.coord.get(id)).filter(Boolean);
}
...
async function main() {
  ...
  const selected = selectEdgesFromCenterline(osm, fit, centerlinePixels, streetIndex, 42);
  ...
  const tiers = [
    { name: "all", keep: (s) => true },
    { name: "score4", keep: (s) => s.score >= 4 },
    { name: "score5", keep: (s) => s.score >= 5 },
    { name: "score6", keep: (s) => s.score >= 6 },
    { name: "score7", keep: (s) => s.score >= 7 },
    { name: "score8", keep: (s) => s.score >= 8 },
    { name: "score9", keep: (s) => s.score >= 9 },
    { name: "score10", keep: (s) => s.score >= 10 },
    { name: "score12", keep: (s) => s.score >= 12 },
    { name: "km06", keep: (s) => s.km >= 0.06 },
    { name: "km07", keep: (s) => s.km >= 0.07 },
    { name: "km08", keep: (s) => s.km >= 0.08 },
    { name: "km09", keep: (s) => s.km >= 0.09 },
    { name: "km10", keep: (s) => s.km >= 0.10 },
    { name: "km11", keep: (s) => s.km >= 0.11 },
    { name: "km12", keep: (s) => s.km >= 0.12 },
    { name: "hybrid-a", keep: (s) => s.score >= 7 || (s.km >= 0.09 && s.score >= 4) },
    { name: "hybrid-b", keep: (s) => s.score >= 8 || (s.km >= 0.10 && s.score >= 5) },
    { name: "hybrid-c", keep: (s) => s.score >= 9 || (s.km >= 0.11 && s.score >= 5) },
    { name: "upper-detail", keep: (s) => s.score >= 8 || (s.cy < 0.42 && s.km >= 0.08) || (s.cy > 0.64 && s.horizontal > 2.4) },
  ];
  const summary = [];
  let n = 0;
  const modes = ["original", "nearest", "leftToRight", "rightToLeft", "banded", "bottomFirst", "topFirst"];
  for (const tier of tiers) {
    const keepStats = stats.filter(tier.keep);
    const keepIds = new Set(keepStats.flatMap((s) => s.comp.edgeIds));
    const keepSelected = selected.filter((e) => keepIds.has(e.index));
    if (keepSelected.length < 8) continue;
    for (const mode of modes) {
      const chain = stitchSelectedWalkMode(osm, keepSelected, mode);
      if (chain.length < 2) continue;
      const km = routeLengthKm(chain);
      const id = `topo-${String(++n).padStart(3, "0")}`;
      const dir = path.join(outDir, id);
      await fs.mkdir(dir, { recursive: true });
      const blind = path.join(dir, "route-blind.png");
      const union = path.join(dir, "street-union.png");
      await renderRouteOnly(chain, blind, `${id} ${km.toFixed(1)} km`);
      await renderStreetUnion(osm, keepSelected, chain, union, `${id} ${mode} union ${(keepSelected.reduce((sum, e) => sum + e.w, 0) / 1000).toFixed(1)} km, stitched ${km.toFixed(1)} km`);
      await fs.writeFile(path.join(dir, `${id}.gpx`), gpx(id, chain));
      summary.push({ id, tier: tier.name, mode, km: +km.toFixed(3), selectedEdges: keepSelected.length, components: keepStats.length, selectedKm: +(keepSelected.reduce((sum, e) => sum + e.w, 0) / 1000).toFixed(3), blindImage: path.relative(root, blind).replace(/\\/g, "/"), unionImage: path.relative(root, union).replace(/\\/g, "/"), gpx: path.relative(root, path.join(dir, `${id}.gpx`)).replace(/\\/g, "/") });
    }
  }
  summary.sort((a, b) => Math.abs(a.km - 18.1) - Math.abs(b.km - 18.1));
  await fs.writeFile(path.join(outDir, "component-stats.json"), JSON.stringify(stats.map((s) => ({ i: s.i, km: +s.km.toFixed(3), score: +s.score.toFixed(2), cx: +s.cx.toFixed(3), cy: +s.cy.toFixed(3), horizontal: +s.horizontal.toFixed(2), edges: s.edges.length, nodes: s.comp.nodes.length })), null, 2));
  await fs.writeFile(path.join(outDir, "summary.json"), JSON.stringify({ fit, routePixels: route.pixels.length, selectedEdges: selected.length, selectedKm: +(selected.reduce((sum, e) => sum + e.w, 0) / 1000).toFixed(3), components: stats.length, results: summary }, null, 2));
