...
const outDir = path.join(root, "tmp-logo-proof", "strava-blockstep-agent");
const M = 111320;
...
function uniquePush(out, p) {
  const last = out[out.length - 1];
  if (!last || Math.hypot(last[0] - p[0], last[1] - p[1]) > 0.0001) out.push(p);
}

function stair(a, b, steps, xFirst = true) {
  const out = [a];
  let cur = a;
  for (let i = 1; i <= steps; i++) {
    const next = [a[0] + ((b[0] - a[0]) * i) / steps, a[1] + ((b[1] - a[1]) * i) / steps];
    if (Math.abs(next[0] - cur[0]) < 0.0001 || Math.abs(next[1] - cur[1]) < 0.0001) {
      uniquePush(out, next);
    } else if (xFirst) {
      uniquePush(out, [next[0], cur[1]]);
      uniquePush(out, next);
    } else {
      uniquePush(out, [cur[0], next[1]]);
      uniquePush(out, next);
    }
    cur = next;
  }
...
function blockOutline(vertices, steps = 3) {
  const out = [];
  for (let i = 1; i < vertices.length; i++) {
    ...
    const segment = dx > 0.01 && dy > 0.01 ? stair(a, b, steps, i % 2 === 0) : [a, b];
    for (const p of segment) uniquePush(out, p);
  }
  return out;
}

// Planned block-step interpretation: big, intentional stair steps instead of
// contour-sampled diagonal wiggles. This is still generic route-native logo
// planning: simplify to meaningful silhouette corners, then route those corners.
const motif = {
  top: blockOutline([
    [0.04, 0.56],
    [0.40, 0.02],
...
<gpx version="1.1" creator="PaceCasso Strava block-step scratch" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name>
${parts
  .map(
    (seg) => `<trkseg>
...
function singleGpx(name, chain) {
  return `<?xml version="1.0" encoding="UTF-8"?>
<gpx version="1.1" creator="PaceCasso Strava block-step scratch" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name><trkseg>
...
async function main() {
  ...
  const candidates = [];
  let checked = 0;
  for (let i = 0; i < centerPool.length; i += 10) {
    const center = centerPool[i].p;
    for (const scale of [1500, 1900, 2400, 3000, 3800, 4800]) {
      for (const rot of [-8, 0, 8, 90, -90]) {
        for (const squeeze of [0.88, 1.0, 1.15]) {
          checked++;
          ...
          const topSnap = snapPoly(index, topTarget, Math.max(95, scale * 0.075));
          const lowerSnap = snapPoly(index, lowerTarget, Math.max(95, scale * 0.075));
          if (!topSnap || !lowerSnap) continue;
          const topRoute = routeAnchors(graph, index, topSnap.nodes, 3.8);
          const lowerRoute = routeAnchors(graph, index, lowerSnap.nodes, 3.8);
          if (!topRoute || !lowerRoute) continue;
          const km = routeKm([topRoute.chain, lowerRoute.chain]);
          if (km < 8 || km > 35) continue;
