...
const outDir = path.join(root, "tmp-sneaker-ordered-loose-fit", stamp);
...
function gpx(name, chain) {
  return `<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="PaceCasso gap6 sneaker fit" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name><trkseg>\n${chain.map(([lat, lng]) => `<trkpt lat="${lat.toFixed(7)}" lon="${lng.toFixed(7)}"></trkpt>`).join("\n")}\n</trkseg></trk></gpx>\n`;
}
...
async function extractScreenshotRoute() {
  const orderSpec = (process.argv[2] ?? "0,-4,-3,-1,-2")
    .split(",")
    .map((raw) => ({ id: Math.abs(Number(raw.trim())), flip: raw.trim().startsWith("-") }));
  ...
  for (let y = 0; y < info.height; y++) {
    for (let x = 0; x < info.width; x++) {
      if (fullMask[idx(x, y, info.width)]) routePixels.push([x, y]);
    }
  }
  if (!routePixels.length) throw new Error("No screenshot route pixels found.");
  const rb = bounds(routePixels);
  const pad = 8;
  const crop = {
    minX: rb.minX,
    maxX: rb.maxX,
  ...
  const cw = rb.maxX - rb.minX + 1 + pad * 2;
  const ch = rb.maxY - rb.minY + 1 + pad * 2;
  const lineMask = new Uint8Array(cw * ch);
  for (const [x, y] of routePixels) {
    lineMask[idx(x - rb.minX + pad, y - rb.minY + pad, cw)] = 255;
  }
  const closedComponents = prepareTracedBinaryComponents(lineMask, cw, ch, 6, 20);
  const polylines = closedComponents
    .map((comp) => centerlinePolylineFromPreparedBinary(comp, cw, ch))
    .filter((path) => path && path.length >= 2);
  const joined = [];
  for (const part of orderSpec) {
    const polyline = polylines[part.id];
    if (!polyline) throw new Error(`Order references missing component ${part.id}; have ${polylines.length}`);
    const points = part.flip ? [...polyline].reverse() : [...polyline];
    joined.push(...points);
  }
  const cleanJoined = simplifyByDistance(joined, 1.8);
  const b = bounds(cleanJoined);
  const width = b.maxX - b.minX || 1;
  const unit = cleanJoined.map(([x, y]) => [(x - b.minX) / width, (y - b.minY) / width]);
...
async function main() {
  ...
  const latticeFile = process.argv[3] ?? path.join(root, "tmp-city-lattice", "manhattan-lattice.json");
  const lattice = JSON.parse(await fs.readFile(latticeFile, "utf8"));
  ...
  const widths = [1300, 1500, 1700, 1900, 2200, 2500];
  ...
  const compiled = [];
  let idCounter = 0;
  for (const c of scanned.slice(0, 20)) {
    const placed = placePath(design.normalized, c);
    const result = compileContourToLattice(placed, graph, {
      sampleMeters: 72,
      pinRadiusMeters: 190,
      minPinSpacingMeters: 125,
      maxLegDetourRatio: 2.25,
      maxLegDetourSlackMeters: 240,
    });
    if (!result) continue;
    ...
    const lengthPenalty = Math.max(0, Math.abs(result.km - 18.1) - 1.1) * 10;
    const score =
      c.score +
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ latticeFile: path.relative(root, latticeFile), design: { crop: design.crop, size: design.size, polylines: design.polylines, points: design.points.length, scoreSamples: design.scoreSamples.length, heightRatio: design.heightRatio }, scanned: scanned.length, top: summary }, null, 2),
  );
