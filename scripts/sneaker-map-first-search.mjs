...
const outDir = path.join(root, "tmp-sneaker-map-first", stamp);
...
async function main() {
  ...
  await renderPath(design.routePoints, path.join(outDir, "1-reference-derived-design.png"));
  const segs = buildStreetSegments(graph);
  const segIndex = segmentGrid(segs);
  const centers = candidateCenters(graph);
  const rotations = [-28, -20, -12, -6, 0, 6, 12, 20, 28];
  const widths = [1800, 2300, 2900, 3600, 4400, 5400, 6600];
  const fabric = [];
  let scanCount = 0;
  ...
  const compilePool = fabric.slice(0, 180);
  const compiled = [];
  let id = 0;
  for (const c of compilePool) {
    ...
    const result = compileContourToLattice(placed, graph, {
      sampleMeters: 28,
      pinRadiusMeters: 150,
      minPinSpacingMeters: 55,
      maxLegDetourRatio: 2.6,
      maxLegDetourSlackMeters: 230,
    });
    if (!result) continue;
  ...
  await fs.writeFile(path.join(outDir, "summary.json"), JSON.stringify({ scanCount, centers: centers.length, segmentCount: segs.length, design: { crop: design.crop, routePoints: design.routePoints.length, featureStrokes: design.featureStrokes.map((s) => s.length) }, top: summary }, null, 2));
