...
const outDir = path.join(root, "tmp-sneaker-gap-routed", new Date().toISOString().replace(/[:.]/g, "-"));
...
async function main() {
  ...
  const raw = parseGpx(await fs.readFile(path.join(sourceDir, "compact-071-raw.gpx"), "utf8"));
